...
function httpc.request(method, host, url, recvheader, header, content)
	local protocol, host = check_protocol(host)
	...
	if not hostname:match(".*%d+$") then
		hostname = socket.resolve(hostname)
	end
	-- TODO: loop connect ?
	local fd, err = socket.connect(hostname, port)
	if not fd then
		error(string.format("%s connect error host:%s, port:%s, err:%s", protocol, hostname, port, err))
		return
	end
